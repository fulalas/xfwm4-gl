...
Vsync changes too, in a smaller way. `xfwm4` already had it, applied to the
image it copied to the screen; here it belongs to the OpenGL renderer and is
applied to the frames it presents. The practical difference is that adaptive
vsync can now be asked for, so a frame that is already late is shown right away
instead of waiting for the next refresh. See Settings below.
